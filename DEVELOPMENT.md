...
この環境整備PRが未マージなら、以下でそのブランチを取得できます。

```powershell
git fetch origin codex/multi-pc-development-20260829
git switch --track origin/codex/multi-pc-development-20260829
```

マージ後はmainにこれらのファイルが含まれるため、上記のブランチ切替は不要です。
既にローカルに対象ブランチがあれば `git switch codex/multi-pc-development-20260829`
を使います。変更があるときは勝手に捨てず停止してください。
...
- 土曜10:00 JSTの週報PDF配信はPR #2が未マージの間は未稼働です。
  状況は [PR #2](https://github.com/hongsan13/discord-market-bot/pull/2) で確認してください。
